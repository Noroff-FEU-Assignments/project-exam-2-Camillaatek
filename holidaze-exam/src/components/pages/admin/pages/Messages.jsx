...
const Messages = () => {
  ...
  return (
      ...
      {contacts.map((item, idx) => {
        return (
         <Link to={`/contact/${item.id}`} style={{ color: 'inherit', textDecoration: 'inherit'}} className="link">
              <div className="messages__card" key={idx}>
            
              <div className='messages__nametime'>
                <p>{item.attributes.name}</p>
                <p className='date'>{format(new Date(item.attributes.createdAt), 'HH:MM Do MMMM YYYY')}</p>
              </div>
              <p className='subject'>
                {item.attributes.subject}</p>
